...
export class TrendsService implements OnModuleInit, OnModuleDestroy {
  ...
  private async aggregateInMongo(
    ...
    const minFields: any[] = fields.map((f) => ({ $toDouble: `$${f}` }));
    const maxFields: any[] = fields.map((f) => ({ $toDouble: `$${f}` }));
    const avgFields: any[] = fields.map((f) => ({ $toDouble: `$${f}` }));
    ...
    const aggData = await this.pqCollection
      .aggregate([
        { $match: { timestamp: { $gte: start, $lte: end } } },
        {
          $group: {
            _id: { $dateTrunc: { date: '$timestamp', unit: groupBy } },
            min: { $min: { $min: minFields } },
            max: { $max: { $max: maxFields } },
            avg: { $avg: { $avg: avgFields } },
          },
        },
        {
          $project: {
  ...
  private fillMissingIntervals(
    ...
    const dataMap = new Map(
      data.map((d) => [new Date(d.interval).getTime(), d]),
    );
    let cursor = DateTime.fromJSDate(start).setZone('Asia/Karachi');
    const endTime = DateTime.fromJSDate(end).setZone('Asia/Karachi');
    ...
    while (cursor <= endTime) {
      ...
      switch (groupBy) {
        case 'hour':
          key = cursor.startOf('hour');
          cursor = cursor.plus({ hours: 1 });
          break;
        case 'day':
          key = cursor.startOf('day');
          cursor = cursor.plus({ days: 1 });
          break;
        case 'month':
          key = cursor.startOf('month');
          cursor = cursor.plus({ months: 1 });
          break;
      }

      const existing = dataMap.get(key.toJSDate().getTime());
      filled.push(
        existing ?? {
          interval: key.toFormat('yyyy-MM-dd HH:mm:ss'), // string in Asia/Karachi
          parameter,
          min: null,
          max: null,
          avg: null,
  ...
  async getTrend(parameter: 'Voltage' | 'Current' | 'Power', interval: string) {
    ...
    switch (interval) {
      ...
      case 'thisMonth':
        ...
        break;
      case 'last30Days':
        start = now.minus({ days: 30 }).startOf('day').toJSDate();
        end = now.endOf('day').toJSDate();
        groupBy = 'day';
        break;
      case 'thisYear':
